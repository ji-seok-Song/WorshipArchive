...
`PDF 선택 → 원본 보관 → 페이지 분석 → 곡 범위 제안 → 사용자 확인 → 라이브러리 등록 → 검색 → 해당 페이지에서 열기`
...
- 예배 날짜와 예배 종류는 `PerformanceRecord`로 관리해 날짜 검색의 기반으로 사용한다.
...
공유 확장, 사진 가져오기, 문서 스캔, 자동 키 추천, 동일 곡 병합, PDF 내보내기, 필기, 블루투스 페달, 셋리스트 협업, 음원 연결은 핵심 흐름 이후 별도 커밋으로 개발한다.
